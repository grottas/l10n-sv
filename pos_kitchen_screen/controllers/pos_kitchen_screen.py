...
class KitchenScreenBase(http.Controller):
                                ...
                                for categ in line.product_id.pos_categ_id:
                                    if categ in pos_screen_data.pos_category_ids and not count:
                                        ...
                                        if categ_id not in product_category_wise:
                                            product_category_wise[categ_id] = {line.product_id.id:line_vals}
                                        else:
                                            product_category_wise[categ_id].update({line.product_id.id:line_vals})
        ...
        if categ and categ in pos_screen_data.pos_category_ids and not count:
            ...
            if len(product):
                ...
                if len(line_vals['product_id'][1].split(']')) == 2:
                    line_vals['product_id'][1] = line_vals['product_id'][1].split(']')[1]
            else:
                line_vals['product_id'] = [line.product_id.id,line.product_id.name]
    ...
    @http.route('/pos/<int:id>/confirm/order',type='http', auth='none', cors='*', csrf=False, save_session=False)
    ...
    @http.route('/pos/<int:id>/done/order',type='http', auth='none', cors='*', csrf=False, save_session=False)
    ...
    @http.route('/pos/<int:id>/done/orderline',type='http', auth='none', cors='*', csrf=False, save_session=False)
