            ...
            for order_line in order_lines:
                if order_line:
                    ...
                    if "own_line" in order_line[2] and res.get('to_invoice', False):
                        own_pro_list = [process_line(line) for line in order_line[2]["own_line"]] if \
                        order_line[2]["own_line"] else False
                        if own_pro_list:
            ...
            for order_line in range(len(new_order_line)):
                if len(ui_order['lines']) > order_line:
                    if ui_order['lines'][order_line] and ui_order['lines'][order_line][2].get('sale_order_origin_id'):
                        new_order_line[order_line][2]['sale_order_origin_id'] = ui_order['lines'][order_line][2][
                            'sale_order_origin_id']
                    if ui_order['lines'][order_line][2].get('sale_order_line_id'):
                        new_order_line[order_line][2]['sale_order_line_id'] = ui_order['lines'][order_line][2][
                            'sale_order_line_id']
